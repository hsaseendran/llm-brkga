// configs/rcmadp_interleaved_config.hpp
// Resource-Constrained Multi-Agent Drop-and-Pick Problem (RCMADP) - Interleaved Version
//
// This version allows fully interleaved dropoff and pickup operations, where the
// chromosome directly controls the unified sequence of all operations.
//
// Problem Description:
...
//
// Key Difference from Standard RCMADP:
// - Standard version: Separate dropoff phase then pickup phase
// - Interleaved version: Unified operation sequence (D1, P1, D2, D3, P2, P3, etc.)
//
...
#ifndef RCMADP_INTERLEAVED_CONFIG_HPP
#define RCMADP_INTERLEAVED_CONFIG_HPP
...
#define RCMADP_INTERLEAVED_MAX_CUSTOMERS 256
#define RCMADP_INTERLEAVED_MAX_AGENTS 16
#define RCMADP_INTERLEAVED_MAX_OPS (2 * RCMADP_INTERLEAVED_MAX_CUSTOMERS)

// Operation type enum
enum class OpType { DROPOFF = 0, PICKUP = 1 };

// GPU kernel for interleaved RCMADP fitness evaluation
template<typename T>
__global__ void rcmadp_interleaved_fitness_kernel(
    ...
    int op_order[RCMADP_INTERLEAVED_MAX_OPS];
    T op_vals[RCMADP_INTERLEAVED_MAX_OPS];

    for (int i = 0; i < num_ops && i < RCMADP_INTERLEAVED_MAX_OPS; i++) {
        op_order[i] = i;
        op_vals[i] = op_priorities[i];
    }

    // Sort operations by priority (insertion sort)
    for (int i = 1; i < num_ops && i < RCMADP_INTERLEAVED_MAX_OPS; i++) {
        int j = i;
    ...
    // Agent state
    T agent_time[RCMADP_INTERLEAVED_MAX_AGENTS];
    int agent_loc[RCMADP_INTERLEAVED_MAX_AGENTS];
    int agent_res[RCMADP_INTERLEAVED_MAX_AGENTS];

    for (int a = 0; a < num_agents && a < RCMADP_INTERLEAVED_MAX_AGENTS; a++) {
        agent_time[a] = 0;
        agent_loc[a] = 0;  // Depot
    ...
    T dropoff_time[RCMADP_INTERLEAVED_MAX_CUSTOMERS + 1];
    bool dropped[RCMADP_INTERLEAVED_MAX_CUSTOMERS + 1];
    bool picked_up[RCMADP_INTERLEAVED_MAX_CUSTOMERS + 1];

    for (int i = 0; i <= n && i <= RCMADP_INTERLEAVED_MAX_CUSTOMERS; i++) {
        dropoff_time[i] = -1;
        dropped[i] = false;
        picked_up[i] = false;
    }
    ...
    bool op_done[RCMADP_INTERLEAVED_MAX_OPS];
    for (int i = 0; i < num_ops && i < RCMADP_INTERLEAVED_MAX_OPS; i++) {
        op_done[i] = false;
    }
    ...
    int unserviced = 0;
    for (int c = 1; c <= n && c <= RCMADP_INTERLEAVED_MAX_CUSTOMERS; c++) {
        if (!dropped[c]) unserviced++;
        if (!picked_up[c]) unserviced++;
    }

    // Fitness = total travel + penalty for unserviced operations
    fitness[idx] = total_travel + unserviced * penalty_weight;
}


template<typename T>
class RCMADPInterleavedConfig : public BRKGAConfig<T> {
public:
    // Problem parameters
...
private:
    ...
    struct AgentSchedule {
        int agent_id;
        std::vector<std::tuple<int, OpType, T>> operations;  // (customer_id, type, time)
        T total_travel_time;
        T finish_time;
    ...
    struct Solution {
        ...
        std::vector<std::pair<int, OpType>> unserviced_list;
    };

public:
    RCMADPInterleavedConfig(int agents = 3, int resources = 2)
    ...
    void evaluate_population_gpu(T* d_population, T* d_fitness,
        ...
        if (!d_travel || !d_proc) {
            std::cerr << "RCMADP Interleaved GPU: memory not allocated on device " << device_id << std::endl;
            return;
        }
        ...
        rcmadp_interleaved_fitness_kernel<<<grid, block>>>(
        ...
        if (error != cudaSuccess) {
            std::cerr << "RCMADP Interleaved GPU kernel failed on device " << device_id
    ...
    int apply_local_search_gpu(
        ...
        rcmadp_interleaved_fitness_kernel<<<eval_grid, eval_block>>>(
    ...
    static std::unique_ptr<RCMADPInterleavedConfig<T>> load_from_tspj(
            const std::string& filename,
            int num_agents = 3,
            int resources_per_agent = 2) {

        auto config = std::make_unique<RCMADPInterleavedConfig<T>>(num_agents, resources_per_agent);
    ...
    static std::unique_ptr<RCMADPInterleavedConfig<T>> load_from_files(
...
public:
    ...
    Solution decode_solution(const Individual<T>& individual) const {
                ...
                if (feasible && best_agent >= 0) {
                    ...
                    if (is_dropoff) {
                        ...
                        sol.agent_schedules[best_agent].operations.push_back(
                            {customer, OpType::DROPOFF, agent_time[best_agent]});
                    } else {
                        T ready_time = dropoff_times[customer] + processing_times[customer];
                        ...
                        sol.agent_schedules[best_agent].operations.push_back(
                            {customer, OpType::PICKUP, agent_time[best_agent]});
                    }
        ...
        for (int c = 1; c <= num_customers; c++) {
            if (!dropped[c]) {
                ...
                sol.unserviced_list.push_back({c, OpType::DROPOFF});
            }
            if (!picked_up[c]) {
                sol.unserviced_pickups++;
                sol.unserviced_list.push_back({c, OpType::PICKUP});
            }
        }
    ...
    std::vector<std::vector<T>> decode_to_solution_vector(const Individual<T>& individual) const {
        ...
        for (const auto& schedule : sol.agent_schedules) {
            ...
            for (const auto& [customer, type, time] : schedule.operations) {
                agent_ops.push_back(type == OpType::DROPOFF ? T(0) : T(1));
    ...
    void print_solution(const Individual<T>& individual) override {
        ...
        std::cout << "\n=== RCMADP Interleaved Solution ===" << std::endl;
        ...
        for (const auto& schedule : sol.agent_schedules) {
            ...
            for (const auto& [customer, type, time] : schedule.operations) {
                std::cout << (type == OpType::DROPOFF ? "D" : "P")
        ...
        if (!sol.unserviced_list.empty()) {
            ...
            for (const auto& [customer, type] : sol.unserviced_list) {
                std::cout << (type == OpType::DROPOFF ? "D" : "P") << customer << " ";
            }
            std::cout << std::endl;
        }
    }
    ...
    void export_solution(const Individual<T>& individual, const std::string& filename) override {
        ...
        file << "RCMADP Interleaved Solution\n";
        ...
        for (const auto& schedule : sol.agent_schedules) {
            ...
            for (const auto& [customer, type, time] : schedule.operations) {
                file << "    " << (type == OpType::DROPOFF ? "DROPOFF" : "PICKUP")
                     << " Customer " << customer
    ...
    void print_instance_info() const {
        std::cout << "\n=== RCMADP Interleaved Instance ===" << std::endl;
        ...
        std::cout << "Chromosome Length: " << (4 * num_customers) << std::endl;
        std::cout << "  - Operation priorities: " << (2 * num_customers) << " genes" << std::endl;
        std::cout << "  - Agent hints: " << (2 * num_customers) << " genes" << std::endl;
    ...
    static void configure_for_size(RCMADPInterleavedConfig<T>* config, int num_customers, int num_agents) {
        int complexity = num_customers * num_agents;
...
#endif // RCMADP_INTERLEAVED_CONFIG_HPP
