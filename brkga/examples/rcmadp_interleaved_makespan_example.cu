// examples/rcmadp_interleaved_makespan_example.cu
// Example usage of the Interleaved RCMADP solver with MAKESPAN optimization
//
// This version minimizes makespan (completion time) instead of total travel cost.
//
// Compile:
//   nvcc -O3 -std=c++17 -arch=sm_75 -Xcompiler -fopenmp -I.. \
//       rcmadp_interleaved_makespan_example.cu -o rcmadp_interleaved_makespan_solver \
//       -lcurand -lcudart
//
// Run:
//   ./rcmadp_interleaved_makespan_solver --tspj <TT_file> [num_agents] [resources_per_agent] [options]
//
// Example:
//   ./rcmadp_interleaved_makespan_solver --tspj ../data/berlin52_TSPJ_TT.csv 4 3 --pop 1000 --gen 500

#include "configs/rcmadp_interleaved_makespan_config.hpp"
...
void print_usage(const char* program) {
    std::cout << "RCMADP Interleaved Makespan Solver - Minimizes completion time\n\n";
    std::cout << "Usage:\n";
...
int main(int argc, char* argv[]) {
    ...
    std::unique_ptr<RCMADPInterleavedMakespanConfig<Float>> config;
    int num_agents = 3;
    ...
    try {
        std::cout << "Loading RCMADP Interleaved instance (Makespan Optimization)...\n";
        ...
        if (std::string(argv[1]) == "--tspj") {
            ...
            config = RCMADPInterleavedMakespanConfig<Float>::load_from_tspj(
            ...
            config = RCMADPInterleavedMakespanConfig<Float>::load_from_files(
                tsp_file, proc_file, num_agents, resources_per_agent);
        }

        config->print_instance_info();

        RCMADPInterleavedMakespanConfig<Float>::configure_for_size(
        ...
        std::cout << "  Objective: MINIMIZE MAKESPAN\n";

        RCMADPInterleavedMakespanConfig<Float>* config_ptr = config.get();

        std::cout << "\nStarting optimization (makespan minimization)...\n";
        ...
        std::cout << "Best Fitness (Makespan): " << best.fitness << "\n";
